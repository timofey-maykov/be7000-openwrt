...
Текущая версия 1.2.5. Образы лежат в Releases, проверяйте суммы по sha256sums.txt. Что изменилось после 1.1, описано ниже в разделе про версии.
...
После загрузки роутер отвечает на 192.168.1.1, пароль root пустой. Начиная со сборки после 1.2.5 при чистой установке поднимается Wi-Fi на обоих диапазонах, сеть OpenWrt-BE7000, пароль be7000openwrt, так что зайти можно и без кабеля. В 1.2.5 и старше Wi-Fi при установке выключен. Первым делом задайте пароль root и смените пароль Wi-Fi. Если подключаетесь с Mac, адрес адаптеру задавайте вручную и без шлюза. Иначе macOS отправит весь трафик в роутер, у которого ещё нет интернета.
...
На образах до 1.2.4 включительно у wget нет https, и команда выше заканчивается ошибкой SSL. Тогда проще так: обнулить счётчики руками и сразу обновиться через sysupgrade, семи загрузок запаса на это хватит с большим избытком.
...
Служба be7000-wifi-defaults при чистой установке включает оба радио с сетью OpenWrt-BE7000 и паролем be7000openwrt, при обновлении настроек не трогает. Исходник лежит в overlay-files.
...
На образ, где этой страницы ещё нет, она ставится пакетом из фида: apk add luci-app-be7000-update (нужны сертификаты в образе, то есть 1.2.5 и новее). Версия образа лежит в /etc/be7000-release, её пишет CI при сборке. Образ без этого файла считается dev и обновление себе не предлагает. Нужен доступ к github.com и api.github.com по https, поэтому работает начиная с 1.2.5, где в образе появились сертификаты.
...
**1.2.6**, 25 сентября 2026. bigoverlay больше не форматирует раздел настроек стока: overlay живёт каталогом внутри тома cfg самого стока, откат сохраняет настройки стока и root по SSH, возврат на OpenWrt сохраняет настройки OpenWrt (предложение FOV5). Проверка и установка обновлений из GitHub Releases, команда be7000-update и страница Система, Обновление сборки. Две службы под обновления с переживающим их overlay: be7000-feeds на каждой загрузке приводит путь к фиду модулей ядра к хешу ядра из ROM (customfeeds.list это conffile, sysupgrade оставлял старый путь), be7000-romsync при смене образа сбрасывает копию базы apk из overlay на базу из ROM и переустанавливает пакеты, которые пользователь ставил сверх образа (иначе apk после обновления считал установленным старое ядро и отказывался ставить модули). Третья часть той же истории, найденная на своём роутере: модули ядра, поставленные через apk, лежат в overlay под тем же номером версии ядра, что и у нового образа, и после смены образа перекрывают модули из ROM. OpenWrt не проверяет их совместимость, они грузятся и валят ядро секунд через двадцать после старта, загрузчик после шести таких попыток уходит в другой слот. Теперь preinit-хук 79_be7000_stale_modules до монтирования overlay сравнивает хеш ядра в копии базы apk с ядром ROM и при расхождении откладывает каталог модулей в lib/modules.stale-<хеш>, а be7000-romsync потом ставит нужные модули из фида заново. Wi-Fi при чистой установке теперь включён, сеть OpenWrt-BE7000 с паролем be7000openwrt: у двух плат после установки 1.2.5 система загрузилась, а по кабелю на 192.168.1.1 не отвечала, и раз система подтвердила загрузку, загрузчик в сток больше не вернёт, без второго входа это тупик. Журнал во флеше теперь кроме dmesg пишет ip addr, конфиг network, состояние board.json и хвост logread, и делает это через обычный ip, у busybox нет ключа -br, из-за чего блок с интерфейсами в 1.2.5 оставался пустым. Пустой /etc/board.json удаляется в preinit, чтобы board_detect собрал его заново. Docker: пакеты, модули и страницы уже в 1.2.4, осталось проверить вживую.

**1.2.5**, 25 сентября 2026. Чистая установка со стока в 1.2.3 и 1.2.4 отскакивала обратно в сток: bigoverlay перезагружала роутер раньше, чем be7000-bootconfirm подтверждал пробную загрузку, и загрузчик возвращал сток. Теперь bigoverlay заново взводит пробную загрузку перед своей перезагрузкой. Тема Nimbus вернулась в образ, CI её не собирал. В образе появились ca-bundle и libustream-mbedtls, без них apk update по https падал с ошибкой сертификата, а wget не умел https, это было и в 1.2.3.

**1.2.4**, 25 сентября 2026. Первый релиз, собранный целиком на GitHub Actions. В фиде появились пакеты и модули под Docker, команда be7000-docker и страницы Docker в LuCI в образе есть, но вживую ещё не проверялись. bigoverlay ставит стоку флаг flag_format_overlay=1. Образ пишет журнал загрузки во флеш: dmesg в crash_syslog (mtd27) в начале и в конце загрузки, паники ядра через mtdoops в crash (mtd26). Читается со стока через dd и strings, подробности в инструкции, это способ разбираться с платами, которые не грузятся, без UART. Без него сток после возврата видел на mtd28 чужой UBI, удалял наш том, создавал пустой data и не копировал свои настройки, в итоге поднимался с пустым /etc/config, без веб-настроек и без SSH (в xmir-patcher это выглядело как «dropbear is found, but it cannot run»). Кто уже попал: на стоке nvram set restore_defaults=1 && nvram commit && reboot -f, или reset кнопкой. rollback-to-stock.sh ставит тот же флаг.
