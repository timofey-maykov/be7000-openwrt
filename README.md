...
**1.2.4**, 25 сентября 2026. bigoverlay ставит стоку флаг flag_format_overlay=1. Без него сток после возврата видел на mtd28 чужой UBI, удалял наш том, создавал пустой data и не копировал свои настройки, в итоге поднимался с пустым /etc/config, без веб-настроек и без SSH (в xmir-patcher это выглядело как «dropbear is found, but it cannot run»). Кто уже попал: на стоке nvram set restore_defaults=1 && nvram commit && reboot -f, или reset кнопкой. rollback-to-stock.sh ставит тот же флаг.
