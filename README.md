...
Версия образа лежит в /etc/be7000-release, её пишет CI при сборке. Образ без этого файла считается dev и обновление себе не предлагает. Нужен доступ к github.com и api.github.com по https, поэтому работает начиная с 1.2.5, где в образе появились сертификаты.
