...
Не работает Wi-Fi 7 в режиме точки доступа, драйвер ath12k заявляет EHT только для режима клиента, hostapd падает на установке маяка. Автовыбор канала на 5 ГГц с обычным HE80 работает нормально, проверял несколько раз подряд. Ошибка автовыбора, которую я поначалу принял за общую проблему ACS, возникает только при попытке включить EHT80, то есть это тот же самый первый пункт про Wi-Fi 7, а не отдельная проблема.
