...
Сначала про версию стока. Проверено на 1.1.38, и это не просто так. Двое, у кого система после установки не стартовала, сидели на старом стоке, один на 1.1.16, второй на 1.1.7 после отката через MIWIFIRepairTool. У первого всё заработало сразу после обновления стока до 1.1.38. Загрузчик приезжает вместе с заводской прошивкой, поэтому если у вас что-то старее, сначала обновите сток штатно.
...
Из работающего OpenWrt запустите scripts/rollback-to-stock.sh и перезагрузитесь, он переключает флаги на другой слот. Если система не загружается, выключите и включите питание четыре раза подряд, каждый раз давая ей минуту, загрузчик возьмёт другой слот. Если и это не помогло, остаётся TFTP-режим загрузчика Xiaomi.

Про откат по питанию есть оговорка. Установщик до версии 1.2.2 включительно сразу после записи ставил flag_boot_success=1 и переводил flag_last_success на новый слот, то есть сообщал загрузчику, что новая система уже загрузилась, хотя она ещё не стартовала ни разу. Загрузчик после этого не считал неудачные попытки, и откат по питанию не срабатывал. Двое так и застряли. С 1.2.3 установщик оставляет загрузку неподтверждённой, flag_last_success держит на слоте стока, а подтверждает загрузку сама OpenWrt службой be7000-bootconfirm в самом конце старта. Если система не поднялась, загрузчику есть куда вернуться.
